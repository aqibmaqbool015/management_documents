...
const GradientDropdown = ({ options = [], images = [] }) => {
    const [selectedOption, setSelectedOption] = useState(options[0] || "Upload File");
    ...
    return (
        <div className="relative inline-block ">
                                ...
                                selectItem.map((item, index) => {
                                    return (
                                        <span key={index} className='inline-block align-text-bottom mr-2 '>
                                            <img src={item.image} alt='' className='w-[15px] h-[15px] object-contain inline-block ' />
                                        </span>
                                    );
                                })
                            } */}
                            <span className='inline-block align-text-bottom mr-2 '>
                                <img src={images} alt='' className='w-[15px] h-[15px] object-contain inline-block ' />
                            </span>
            ...
            <div class="absolute inset-y-0 right-0 flex items-center pointer-events-none">
                <img src={image.arrow} alt='' className='w-[12px] h-auto object-contain inline-block ' />
            </div>
        </div>
    );
};
